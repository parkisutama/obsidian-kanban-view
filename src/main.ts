...
import { BoardSettingsManager, BOARD_SETTINGS_KEY, BoardSettingsMap } from './helpers/boardSettings';
import { hasFrontmatterKey, hasKanbanStructure, isKanbanFile } from './fileDetection';
...
export default class KanbanPlugin extends Plugin {
  ...
  /** Per-board settings stored externally (for "pure" Markdown kanban boards) */
  boardSettingsManager: BoardSettingsManager;
  ...
  async loadSettings() {
    ...
    // Initialize BoardSettingsManager with any previously stored per-board settings
    const boardSettingsData: BoardSettingsMap = data?.[BOARD_SETTINGS_KEY] ?? {};
    this.boardSettingsManager = new BoardSettingsManager(
      boardSettingsData,
      async (updated) => {
        // Merge back into plugin data and persist
        const current = (await this.loadData()) ?? {};
        await this.saveData({ ...current, [BOARD_SETTINGS_KEY]: updated });
      }
    );

    // Remove the boardSettings key from the global plugin settings
    // so it doesn't pollute the KanbanSettings namespace.
    delete (this.settings as Record<string, unknown>)[BOARD_SETTINGS_KEY];
  }

  async saveSettings() {
    // Preserve existing boardSettings when saving global settings
    const current = (await this.loadData()) ?? {};
    await this.saveData({
      ...current,
      ...this.settings,
    ...
    if (this.stateManagers.has(file)) {
      ...
      this.stateManagers.set(
        ...
        new StateManager(
          ...
          () => this.settings,
          // External per-board settings callbacks (for pure Markdown boards)
          () => this.boardSettingsManager.get(file.path),
          (settings) => this.boardSettingsManager.replace(file.path, settings)
        )
      );
    }
  ...
  handleViewFileRename(view: KanbanView, oldPath: string) {
    ...
    // Carry over kanbanFileModes from old path to new path so that
    // any subsequent setViewData / setViewState calls still recognise
    // this file as a kanban board.
    const newPath = view.file.path;
    const leafId = view.leaf.id;

    if (this.kanbanFileModes[oldPath] !== undefined) {
      this.kanbanFileModes[newPath] = this.kanbanFileModes[oldPath];
      delete this.kanbanFileModes[oldPath];
    }
    // Always ensure the leaf-id entry is up-to-date so the faster
    // leafId-based lookup in setViewData succeeds.
    this.kanbanFileModes[leafId] = kanbanViewType;

    // Notify DragDropApp so it re-renders portals with the updated
    // view.id (which includes the new file path).
    reg.viewStateReceivers.forEach((fn) => fn(this.getKanbanViews(win)));

    if (view.isPrimary) {
      const stateManager = this.getStateManager(view.file);
      // forceRefresh re-parses all items and re-compiles settings,
      // ensuring the board state is fully consistent after the move.
      stateManager.forceRefresh();
    }
  }
  ...
  registerEvents() {
    ...
    this.registerEvent(
      this.app.vault.on('rename', (file, oldPath) => {
        ...
        // Keep external board settings in sync when a board file is renamed
        if (file instanceof TFile) {
          this.boardSettingsManager.rename(oldPath, file.path).catch((e) =>
            console.error('[Kanban] Failed to rename board settings key', e)
          );
        }
      })
    );

    // FR-06: Clean up orphan board settings when a board file is deleted
    this.registerEvent(
      this.app.vault.on('delete', (file) => {
        if (file instanceof TFile) {
          // Remove stateManager for the deleted file
          this.stateManagers.delete(file);

          // Remove external board settings so they don't accumulate in data.json
          this.boardSettingsManager.remove(file.path).catch((e) =>
            console.error('[Kanban] Failed to remove board settings for deleted file', e)
          );
        }
      })
    );
  ...
  registerCommands() {
    ...
    this.addCommand({
      id: 'open-as-kanban-board',
      name: t('Open current list as Kanban board'),
      checkCallback: (checking) => {
        const activeFile = this.app.workspace.getActiveFile();
        const activeView = this.app.workspace.getActiveViewOfType(MarkdownView);

        if (!activeFile || !activeView) return false;

        const qualifies = hasKanbanStructure(activeFile, this.app);

        if (checking) return qualifies;
    ...
    this.addCommand({
      ...
      checkCallback: (checking) => {
        ...
        const fileIsKanban = isKanbanFile(activeFile, this.app);
  ...
  registerMonkeyPatches() {
    ...
    // Monkey-patch retained: forward hotkeys to active KanbanView
    ...
    // Monkey-patch retained: sync activeEditor for KanbanView
    this.register(
    ...
    // Monkey-patch retained: No official pre-detach event exists.
    this.register(
        ...
        setViewState(next) {
          return function (state: ViewState, ...rest: any[]) {
            if (
              ...
              // Check using the combined detection: frontmatter key OR structural (H2 + checklist)
              const filePath = state.state.file as string;
              const cache = self.app.metadataCache.getCache(filePath);
              const fileObj = self.app.vault.getAbstractFileByPath(filePath);

              const hasLegacyKey =
                cache?.frontmatter && cache.frontmatter[frontmatterKey];
              const hasPureStructure =
                fileObj instanceof TFile && !hasFrontmatterKey(fileObj, self.app) &&
                // Only auto-open as kanban if the user has explicitly done so before
                // (to avoid hijacking every markdown checklist file).
                self.kanbanFileModes[this.id || filePath] === kanbanViewType;

              if (hasLegacyKey || hasPureStructure) {
                // Force the view type to kanban
                const newState = {
                  ...state,
                  type: kanbanViewType,
                };
