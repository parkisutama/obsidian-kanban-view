...
export const Kanban = ({ view, stateManager }: KanbanProps) => {
  ...
  // ── Pretty Properties: DOM-level refresh ────────────────────────
  const refreshPP = useCallback(() => {
    if (rootRef.current) refreshPrettyProperties(rootRef.current);
  }, []);

  // Refresh after view switch — wait one frame for the new DOM to settle
  useEffect(() => {
    const win = view.getWindow();
    const id = win.requestAnimationFrame(refreshPP);
    return () => win.cancelAnimationFrame(id);
  }, [boardView, refreshPP, view]);

  // Refresh when PP (or any plugin) fires css-change
  useEffect(() => {
    const ref = view.app.workspace.on('css-change', refreshPP);
