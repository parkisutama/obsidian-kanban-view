...
export function useTableColumns(boardData: Board, stateManager: StateManager) {
  ...
  const { items, metadata, fileMetadata, metadataLabels } = useTableData(boardData, stateManager);
  ...
  const withFileMetadata = useMemo(() => {
        ...
        columnHelper.accessor(
          ...
          },
          {
            ...
            cell: (info) => {
              ...
              return <MetadataValue data={val} propertyKey={key} searchQuery={searchQuery} />;
            },
            sortDescFirst: false,
            sortingFn: (a, b, id) => {
    ...
    return columns;
  }, [withMetadata, ...fileMetadata, ...metadataLabels.values()]);
