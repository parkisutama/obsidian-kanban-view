...
try {
  ...
  console.log('.env file error:', e.message);
}
...
const renamePlugin = {
  ...
  setup(build) {
    build.onEnd(() => {
      ...
      // Handle both outfile and outdir configurations
      let outcss, fixcss;
      if (outfile) {
        outcss = outfile.replace(/\.js$/, '.css');
        fixcss = outfile.replace(/main\.js$/, 'styles.css');
      } else if (outdir) {
        outcss = path.join(outdir, 'main.css');
        fixcss = path.join(outdir, 'styles.css');
      } else {
        return;
      }

      if (fs.existsSync(outcss) && outcss !== fixcss) {
