...
describe('FT-2.4.7 — pure markdown roundtrip fidelity (no modifications)', () => {
   ...
   * boardToMd() produces:
   *   ## Heading\n\n- [ ] item\n   (one blank line between heading and items)
   *
  ...
  it('preserves normal bullet cards without converting them to checklists', () => {
    const md = '## Lane\n\n- Bullet A\n+ Bullet B\n* Bullet C\n';
    expect(roundtrip(md)).toBe(md);
  });

  it('preserves ordered point markers without converting them to checklists', () => {
    const md = '## Lane\n\n1. First\n2) Second\n';
    expect(roundtrip(md)).toBe(md);
  });

  it('preserves non-list markdown inside a lane before task items', () => {
    const md = [
      '## To Do',
      '',
      'Notes stay with the lane.',
      '',
      '> Including block markdown.',
      '',
      '- [ ] Task A',
      '',
    ].join('\n');
...
describe('FT-2.4.8 — frontmatter preservation', () => {
  ...
  it('plugin does not inject kanban-plugin key into pure-mode frontmatter', () => {
    const md = ['---', 'title: My Board', '---', '', '## Lane', '- [ ] Task', ''].join('\n');
  ...
  it('legacy board (kanban-plugin: board) preserves frontmatter in output', () => {
    const md = ['---', '', 'kanban-plugin: board', '', '---', '', '## Lane', '- [ ] Task', ''].join(
      '\n'
    );
